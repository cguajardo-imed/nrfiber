...
New Relic instrumentation middleware for [Fiber v3](https://docs.gofiber.io/) web framework.
...
func main() {
    ...
    nrApp, err := newrelic.NewApplication(
        newrelic.ConfigAppName("my-fiber-v3-app"),
...
**Parameters:**
- `c` - Fiber context (interface in v3)
...
See the [examples directory](../examples/fiber-v3-basic/) for complete working examples.

## Key Differences from v2

This package is designed for **Fiber v3**. If you're using Fiber v2, use the v2 version instead:
...
func(c fiber.Ctx) error {
    // c is an interface (not a pointer)
    txn := nrfiber.FromContext(c)
    return c.JSON(data)
}
```
...
**v2:**
```go
c.SetUserContext(newrelic.NewContext(c.Context(), txn))
```
